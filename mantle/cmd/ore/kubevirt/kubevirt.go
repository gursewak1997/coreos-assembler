...
func init() {
	KubeVirt.PersistentFlags().StringVar(&configPath, "kubeconfig", "~/.kube/config", "KubeVirt config file")
	KubeVirt.PersistentFlags().StringVar(&options.Container, "kubeconfig", "~/.kube/config", "KubeVirt config file")
...
func preflightCheck(cmd *cobra.Command, args []string) error {
	...
	api, err := kubevirt.New(&kubevirt.Options{
	...
	fmt.Printf("Preflight check success; we have liftoff")
	API = api
	return nil
}
