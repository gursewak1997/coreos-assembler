...
func (kc *cluster) NewMachineWithOptions(userdata *conf.UserData, options platform.MachineOptions) (platform.Machine, error) {
	...
	_, err := kc.RenderUserData(userdata, map[string]string{
		// "$public_ipv4":  "${COREOS_KUBEVIRT_IP_EXTERNAL_0}",
		"$private_ipv4": "${COREOS_KUBEVIRT_IP_LOCAL_0}",
	})
	if err != nil {
		return nil, err
	}

	instance, err := kc.flight.api.CreateInstance(kc.vmname(), conf.String(), kc.sshKey, kc.ResourceGroup, kc.Storagekccount)
	if err != nil {
		return nil, err
	}
